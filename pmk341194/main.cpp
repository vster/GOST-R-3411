...
// Make one test file
void make_one_test( char fname[], int j, int oidnum )
{
	int len;
	byte hashed[HSB];
	byte buf4b[4];
	...
	// Output to file: hash
	testfile.write ( reinterpret_cast<char*>(hashed), HSB);
		
	// Output to file: len of hashed data
	BEFromInt( buf4b, len );
	testfile.write ( reinterpret_cast<char*>(buf4b), sizeof(int) );
	
	// Output to file: hashed data	
	testfile.write ( reinterpret_cast<char*>(TextToHashArray[j]), len);
...
int main() 
{
	...
	cout << "Generation of datafit files for GOST R 34.11-94..." << endl;

	ofstream logfile;
	logfile.open("pmkinfo.txt");
	logfile << "Контрольные тесты ГОСТ Р 34.11-94.\n";
	...
	for ( oidnum = 0, testnum = 1; oidnum < 2; oidnum++)
	{
		get_sbox( oidnum );
		for (int j = 0; j < 9; j++)
		{		
			...
			logfile << fname << " - " << j+1
					<< ", " << OID[oidnum] 
					<< ", " << (int)strlen((char*)TextToHashArray[j])
					<< endl;

			make_one_test( fname, j, oidnum );
			testnum++;	
		}
	}
	logfile.close();
